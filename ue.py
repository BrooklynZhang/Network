from packet import RadarPacket, EchoPacket, DataPacket, AckPacket, HelloPacket, Create_Session_Request
...
class UE(object):
    def __init__(self, env, ue_id, algorithm):
        ...
        self.env.process(self.create_session())
    ...
    def create_session(self):
        tag = 0
        while True:
            packet = Create_Session_Request(self.ue_id, tag)
    ...
    def send_data_packets(self, flow):
        ...
        while flow.num_packets >= 0:
                ...
                packet = DataPacket(flow.src_id, flow.dest_id, flow.dest_id, flow.flow_id, datapacket_id, current_time, flow.ack, 'up') #src_host_id, dest_host_id, flow_id, packetnum, timestamp
